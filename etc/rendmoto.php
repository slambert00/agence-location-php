...
if(!empty($_POST)){
    $req = $connexion->prepare("update location_motos set disponible='free' where immatriculation='$immat'");
    $req->execute();
    $req2 = $connexion->prepare("update location_motos set identifiant_loueur=0 where immatriculation='$immat'");
    $req2->execute();
}
?>
