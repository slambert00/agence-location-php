...
if(!empty($_POST)){
    ...
    if($ligne){
        $req2= $connexion->prepare("update location_voitures set identifiant_loueur=$identifiant where immatriculation='$immat'");
        $req2->execute();
        $req3= $connexion->prepare("update location_voitures set disponible='busy' where immatriculation='$immat'");
        $req3->execute();
        echo "voiture louée!";
    }
    else {
        ...
        $req2->execute();
        $req4= $connexion->prepare("update location_voitures set identifiant_loueur=$hasard where immatriculation='$immat'");
        $req4->execute();
        $req3= $connexion->prepare("update location_voitures set disponible='busy' where immatriculation='$immat'");
        $req3->execute();
